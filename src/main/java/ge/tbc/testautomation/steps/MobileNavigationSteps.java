...
public class MobileNavigationSteps {
    ...
    @Step("Switch to mobile viewport (390x844)")
    ...
    @Step("Validate that primary navigation links have moved to the burger menu")
    public MobileNavigationSteps validateLinksHaveMovedToBurgerMenu() {
        ...
        assertThat(homePage.desktopNavMenu).isHidden();
        homePage.burgerMenuButton.click();
        assertThat(homePage.mobileMenuPanel).isVisible();
        List<String> mainNavLinks = Arrays.asList("What's New", "Women", "Men", "Gear", "Training", "Sale");
